...
const TransactionComponent = (props) =>{
  ...
  const [filteredTransction,updateTxn] = useState(props.transactions);
...
return (
    <Container>
        ...
        <input placeholder="Search" value = {searchText} onChange = {(e)=>{updateSearchText(e.target.value);
          ...
          />
        {filteredTransction?.length ? filteredTransction.map((payload)=> (<TransactionCell payload = {payload}/>
        ))
        :""}
      </Container>
);
